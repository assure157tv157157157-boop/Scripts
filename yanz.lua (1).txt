...
local InfoTab = Window:MakeTab({ Title = "Info", Icon = "rbxassetid://99636054737127" })
...
local TrollTab = Window:MakeTab({ Title = "Scripts Trolls", Icon = "rbxassetid://99636054737127" })


TrollTab:AddSection({ "Black Hole" })
TrollTab:AddButton({
    Name = "Black Hole",
    Description = " Mengaktifkan ini akan menarik Parts ke karakter Anda",
    Callback = function()
...
TrollTab:AddSection({ "Tarik Parts" })
TrollTab:AddButton({
    Name = "Tarik Parts",
    Description = "Untuk menggunakan, dekati Pemain yang Dipilih",
    Callback = function()
        -- Gui to Lua
-- Version: 3.2
...
TrollTab:AddSection({ "Tidak Terlihat" })

TrollTab:AddButton({
    Name = "Jadi Tak Terlihat",
    Description = "Menjadi tak terlihat FE",
Callback = function()
...
TrollTab:AddToggle({
    Name = "RGB Character",
    Description = "Membuat karakter Anda RGB",
    Default = false,
...
TrollTab:AddSection({ "Rambut RGB" })
local hairColors = {
...
TrollTab:AddToggle({
    Name = "Rambut RGB",
    Description = "Berubah Rambut RGB",
    Default = false,
    Callback = function(value)
        isActive = value
        if isActive then
...
TrollTab:AddToggle({
    Name = "Anti Sit",
    Description = "Mencegah karakter Anda duduk",
    Default = false,
    Callback = function(Value)
...
local function KillPlayerCouch()
    if not selectedPlayerName then
        warn("Kesalahan: Tidak ada pemain yang dipilih")
        return
    end
    local target = Players:FindFirstChild(selectedPlayerName)
    if not target or not target.Character then
        warn("Kesalahan: Pemain target tidak ditemukan atau tidak memiliki karakter")
        return
    end

    local char = LocalPlayer.Character
    if not char then
        warn("Kesalahan: Karakter pemain lokal tidak ditemukan")
        return
    end
    ...
    if not hum or not root or not tRoot then
        warn("Kesalahan: Komponen yang diperlukan tidak ditemukan")
        return
    end
...
local function BringWithCouch()
    ...
    if not targetPlayer then
        warn("Kesalahan: Nenhum jogador alvo selecionado")
        return
    end
    if not targetPlayer.Character or not targetPlayer.Character:FindFirstChild("HumanoidRootPart") then
        warn("Kesalahan: Pemain alvo sem personagem ou HumanoidRootPart")
        return
    end
    ...
    if not couch then
        warn("Kesalahan: Sofá não encontrado no Backpack")
        return
    end
    ...
    if seat1 and seat2 and handle then
        ...
        warn("Kesalahan: Componentes do sofá não encontrados")
        return
    end
...
local function KillWithCouch()
    ...
    local killDropdown = Troll:AddDropdown({
        ...
        Callback = function(value)
            ...
            print("Pemain dipilih: " .. tostring(value))
        end
    })
    ...
    Troll:AddButton({
        ...
        Callback = function()
            ...
            if killDropdown and #tablePlayers > 0 then
                ...
                print("Daftar pemain diperbarui: ", table.concat(newPlayers, ", "))
                if selectedPlayerName and not Players:FindFirstChild(selectedPlayerName) then
                    selectedPlayerName = nil
                    getgenv().Target = nil
                    killDropdown:SetValue("")
                    print("Pemilihan direset, pemain tidak lagi di server.")
                end
            else
                print("Kesalahan: Dropdown tidak ditemukan atau tidak ada pemain tersedia.")
            end
        end
    })
    ...
    Troll:AddButton({
        ...
        Callback = function()
            if not selectedPlayerName or not Players:FindFirstChild(selectedPlayerName) then
                print("Kesalahan: Pemain tidak dipilih atau tidak ada")
                return
            end
            local character = LocalPlayer.Character
            local humanoidRootPart = character and character:FindFirstChild("HumanoidRootPart")
            if not humanoidRootPart then
                warn("Kesalahan: HumanoidRootPart do jogador local não encontrado")
                return
            end
            ...
            if targetPlayer and targetPlayer.Character and targetPlayer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                print("Kesalahan: Player alvo não encontrado ou sem HumanoidRootPart")
            end
        end
    })
    ...
    local MethodSection = Troll:AddSection({ Name = "Metode" })

    Troll:AddDropdown({
        Name = "Pilih Metode untuk Membunuh",
                            ...
                            if targetPlayer and targetPlayer.Character and targetPlayer.Character:FindFirstChild("HumanoidRootPart") then
                                ...
                                if targetHumanoid and targetHumanoid.Sit then
                                    if character.Humanoid then
                                        ...
                                        print("Pemain sentou, levando ônibus para o void!")
                                        task.wait(0.2)
...
local function houseBanKill()
    if not selectedPlayerName then
        print("Tidak ada pemain yang dipilih!")
        return
    end
        ...
        if Check() then
            ...
            if not House then
                ...
                if #availableHouses > 0 then
                    ...
                    print("Tidak ada rumah yang tersedia untuk dibeli!")
                    return
                end
            end
...
Troll:AddButton({
    ...
    Callback = function()
        ...
        print("Pemain paralisado para reduzir efeitos do spin.")
        ...
        print("Pemain está preso na coordenada segura.")
        ...
        print("Pemain liberado, seguro na posição.")
    end
})
...
pcall(function()
    HouseTab:AddSection({ Name = "Teleportasi ke Rumah" })
end)
...
-- Label após dropdown
pcall(function()
    HouseTab:AddLabel("Teleportasi ke Rumah yang Kamu Inginkan")
end)
...
pcall(function()
    HouseTab:AddLabel("Secara otomatis membebaskan (unban) dirimu dari Rumah")
end)

-- Seção para Casa RGB
pcall(function()
    HouseTab:AddSection({ Name = "Rumah RGB" })
end)

-- Toggle para Casa RGB
pcall(function()
    HouseTab:AddToggle({
        Name = "Membuat rumah RGB",
        Default = false,
        Callback = function(state)
...
Tab:AddSection({"Di dengar semua player"})
...
local audios = {
    ...
    {Name = "BURNING BUD", ID = 85742937376358},
}
...
Tab:AddTextBox({
    Name = "Masukkan ID Audio atau Musik",
    Description = "Ketik ID audio",
    PlaceholderText = "ID audio",
    Callback = function(value)
...
local SectionNames = Tab:AddSection({
    Name = "Adicionar Nomes no Pemain"
})
...
ChildTab:AddButton({
    ...
    Callback = function()
        if not chasingplayer then
            warn("Tidak ada pemain yang dipilih!")
            return
        end
    ...
    Callback = function(bool)
        ...
        if bool then
            if selectedPlayer and selectedPlayer.Character then
                ...
                warn("Pemain dipilih não encontrado ou sem Character!")
                return false
            end
        else
            if headSitConnection then
